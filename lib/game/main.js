  ...
  ig.module('game.main').requires('impact.game', 'game.gui.frame', 'game.gui.minimap', 'game.levels.main').defines(function() {
    ...
    tj.Main = ig.Game.extend({
      init: function() {
        ...
        this.rBorder = this.backgroundMaps[0].pxWidth - ig.system.width;
        this.bBorder = this.backgroundMaps[0].pxHeight - ig.system.height;
      ...
      update: function() {
        ...
        if (ig.input.state('right')) {
          if (this.screen.x < (this.rBorder + this.tileSize + 266)) {
            this.screen.x += 20;
          } else {
            this.screen.x = this.rBorder + 266 + this.tileSize;
          }
        }
        if (ig.input.state('down')) {
          if (this.screen.y < (this.bBorder + this.tileSize)) {
            this.screen.y += 20;
          } else {
            this.screen.y = this.bBorder + this.tileSize;
          }
        }
      ...
      getDimensions: function() {
        ...
        return this.bOffset = $(document).height() % 32;
      }
    });
